%formula match with input database
function assign_info=pks_dbase(dbase,pks,ppm,mode)
fprintf('find dbase...');
H=1.007276;
[~,A]=sort([pks.mz]);
pks_sorted=pks(A);
...
for i=1:length(pks_sorted)
    ...
    [~,A]=min(abs([assign_info(i).assign.ppm])); %fixed bug,add abs, 4/27/2020
    assign_info(i).formula=assign_info(i).assign(A).Formula;
    assign_info(i).formulaName=assign_info(i).assign(A).Name;  %added
